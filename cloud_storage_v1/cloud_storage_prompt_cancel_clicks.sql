SELECT substr(payload.branch, 8, 26) AS branch, COUNT(payload.data.attributes['message']) AS cancelclicks
...
WHERE payload.study_name = 'cloud-storage-study'
  ...
  AND payload.data.attributes['message'] = 'prompt_cancel_click'
GROUP BY 1
